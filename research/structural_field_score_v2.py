"""Sealed post-prediction scorecard for the fixed-physics v2 structural-field gate.

This module consumes, but never creates or selects among, the fifteen frozen
prediction shards emitted by :mod:`research.structural_field_gate_v2`.  The gate
...
SCORE_VERSION = "geosteern-anchored-structural-field-score/2"
...
def _score_contract() -> dict[str, Any]:
    ...
    return {
        "candidate": "single_frozen_fixed_physics_identity_structural_field_candidate",
        "comparator": "sealed_joint_prediction",
        "candidate_formula": (
            "joint + field_confidence * "
            "(field_delta_without_prefix_bias + prefix_bias_delta)"
        ),
        "fixed_coefficients": {"theta_field": 1.0, "theta_bias": 1.0},
        "calibration_or_grid_selection": "absent_and_forbidden",
        "selection": "forbidden",
...
def _validated_v2_protocol_identity(audit: gate.GateAudit) -> dict[str, Any]:
    """Require and expose the fixed-v2 identity already audited by the gate."""

    protocol = audit.protocol
    if protocol.get("protocol_version") != gate.PROTOCOL_VERSION:
        raise ScoreError("gate protocol version is not fixed-physics v2")
    if protocol.get("method") != gate.METHOD:
        raise ScoreError("gate protocol method is not fixed-physics identity v2")
    fixed = protocol.get("fixed_configuration")
    if fixed != gate.fixed_configuration():
        raise ScoreError("gate protocol fixed-physics identity drift")
    parent = protocol.get("parent_v1_lineage")
    if not isinstance(parent, Mapping) or not _is_sha256(parent.get("lineage_sha256")):
        raise ScoreError("gate protocol lacks sealed v1 STOP parent lineage")
    return {
        "gate_protocol_version": gate.PROTOCOL_VERSION,
        "method": gate.METHOD,
        "fixed_configuration": dict(fixed),
        "parent_lineage_sha256": str(parent["lineage_sha256"]),
    }
...
def _load_sealed_barrier(path: Path) -> tuple[dict[str, Any], str, str]:
    ...
    if (
        payload.get("status") != gate.BARRIER_STATUS
...
def audit_before_truth(
    ...
    try:
        ...
        _validated_v2_protocol_identity(gate_audit)
...
def _validate_audited_output_location(
    output_path: Path,
    audit: gate.GateAudit,
) -> None:
    """Reject fresh score files anywhere inside frozen incumbent/v1 shard trees."""

    frozen_trees = (
        (audit.results_dir / gate.EXACT_SHARD_DIR_NAME).resolve(),
        (audit.results_dir / gate.SPATIAL_SHARD_DIR_NAME).resolve(),
        (audit.results_dir / "anchored_structural_field_protocol_folds").resolve(),
    )
    for target in _output_targets(output_path.resolve()):
        ...
        for tree in frozen_trees:
            try:
                ...
                continue
            raise ScoreError(
                "score output targets must be outside frozen incumbent/v1 shard trees"
            )
...
def _build_result(audit: ScoreAudit, scored: pd.DataFrame) -> dict[str, Any]:
    ...
    identity = _validated_v2_protocol_identity(audit.gate_audit)
    return {
        "score_version": SCORE_VERSION,
        "gate_protocol_version": identity["gate_protocol_version"],
        "method": identity["method"],
        "fixed_configuration": identity["fixed_configuration"],
        ...
        "lineage": {
            ...
            "v1_stop_parent_lineage_sha256": identity["parent_lineage_sha256"],
...
def score_structural_field_v2(
    ...
    _validate_audited_output_location(output, audit.gate_audit)
    ...
    _validate_audited_output_location(output, commit_audit.gate_audit)
...
def audit_score_artifacts_v2(
    ...
    audit = audit_before_truth(protocol_path, benchmark_path, shard_dir, barrier_path)
    _validate_audited_output_location(result_file, audit.gate_audit)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "score":
        result = score_structural_field_v2(
            args.protocol,
            args.benchmark,
            args.shard_dir,
            args.barrier,
        ...
        result = audit_score_artifacts_v2(
            args.result,
            args.protocol,
            args.benchmark,
            args.shard_dir,

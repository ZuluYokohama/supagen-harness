...
from research import structural_field_score_v2 as score


def test_v2_public_score_boundary_has_no_injectable_truth_or_selection() -> None:
    signature = inspect.signature(score.score_structural_field_v2)
    ...
    assert contract["calibration_or_grid_selection"] == "absent_and_forbidden"
    assert contract["selection"] == "forbidden"
    assert contract["pad_out"] == "absent_and_forbidden"
...
def _work_proxy(wells: int) -> dict[str, Any]:
    ...
    for mode, repeat, fold in gate._all_fold_identities():
        training = wells - wells // 5
        validation_queries = 5 if (mode, repeat, fold) == ("exact", 0, 0) else 4
        derivative_mass = 4 * 10 + 10
        node_mass = 4 * 2 + 2
        five_queries = gate.VALIDATION_PROPOSALS_PER_WELL * validation_queries
        identity = f"{mode}:{repeat}:{fold}"
        # Each leave-one role omits exactly one inner block, so the four role
        # counts must sum to 3 * training -- the partition identity the gate
        # enforces. Size the blocks explicitly so it holds for any `wells`.
        blocks = [
            training // 4 + (1 if index < training % 4 else 0) for index in range(4)
        ]
        leave_counts = [training - block for block in blocks]
        rows.append(
            {
                "mode": mode,
                "repeat": repeat,
                "fold": fold,
                "training_wells": training,
                "validation_wells": wells // 5,
                "embargo_wells": 0,
                "inner_role_partition_sha256": "1" * 64,
                "leave_one_training_wells": leave_counts,
                "leave_one_role_sha256": [
                    gate._canonical_digest([identity, "leave", inner])
                    for inner in range(4)
                ],
                "final_role_sha256": "3" * 64,
                "leave_one_derivative_observations": [10] * 4,
                "final_derivative_observations": 10,
                "derivative_fit_mass": derivative_mass,
                "leave_one_fixed_node_count": [2] * 4,
                "final_fixed_node_count": 2,
                "fixed_node_fit_mass": node_mass,
                "validation_support_queries": validation_queries,
                "validation_suffix_rows": (wells // 5) * 4,
                "five_validation_proposal_queries": five_queries,
                "proxy_units": (
                    8 * derivative_mass + 800 * node_mass + 4_096 * five_queries
                ),
            }
        )
...
def _benchmark_payload(
    ...
    timing = {
        ...
        "total": 100.0,
        "extrapolated_two_worker_fifteen_fold": 800.0,
    }
    memory = {"field_peak_rss": 1.0, "total_peak_rss": 2.0}
    maximum = max(work_proxy["folds"], key=gate._work_proxy_order_key)
    fitting_counts = [
        *maximum["leave_one_training_wells"],
        maximum["training_wells"],
    ]
    fitting_digests = [
        *maximum["leave_one_role_sha256"],
        maximum["final_role_sha256"],
    ]
    shape = {
        "lineage_audits": 2,
        ...
        "model_observations": [
            {
                "role": f"leave_one_{index}" if index < 4 else "final",
                "fitting_well_count": fitting_counts[index],
                "fitting_ids_sha256": fitting_digests[index],
    ...
    acceptance = gate._benchmark_acceptance(timing, memory, shape)
    return {
        ...
        "worst_work_identity": {
            ...
            "fold": maximum["fold"],
            "training_wells": maximum["training_wells"],
            "validation_wells": maximum["validation_wells"],
            "embargo_wells": maximum["embargo_wells"],
            "proxy_units": maximum["proxy_units"],
        },
        "work_shape": shape,
...
def _build_synthetic_run(
    ...
    *,
    ...
    truths: dict[str, np.ndarray] = {}
    geometry_by_well: dict[str, tuple[float, ...]] = {}
    ...
    for index, well in enumerate(well_ids):
        ...
        x = index * 10_000.0 + md
        y = index * 100.0 + md * 0.1
        z = 1_000.0 - index - np.arange(6, dtype=float)
        ...
        ).to_csv(horizontal, index=False)
        truths[horizontal_name] = truth
        geometry_by_well[well] = (
            float(x[0]),
            float(y[0]),
            float(z[0]),
            float(x[-1]),
    ...
    protocol_payload = {
        ...
        "method": gate.METHOD,
        "fixed_configuration": gate.fixed_configuration(),
        "parent_v1_lineage": {"lineage_sha256": "9" * 64},
    ...
    for mode, repeat, fold in gate._all_fold_identities():
        ...
        partition = gate._partition_from_geometry(
            training_ids,
            {well: group_by_well[well] for well in training_ids},
            {well: geometry_by_well[well] for well in training_ids},
        )
        ...
        shard = {
            ...
            "fixed_method_from_outer_training_only": {
                "fixed_configuration": gate.fixed_configuration(),
                "field_fit_count": gate.FIELD_FITS_PER_OUTER,
                "validation_proposals_per_well": (gate.VALIDATION_PROPOSALS_PER_WELL),
                "role_partition": partition.metadata,
                "leave_one_field_models": [
                    {
                        "excluded_fold": inner_fold,
                        "model": _model_metadata(
                            int(
                                partition.metadata["leave_one_roles"][inner_fold][
                                    "fitting_well_count"
                                ]
                            )
                        ),
                    }
                    for inner_fold in range(gate.INNER_FOLDS)
                ],
                "final_role": {
                    "fitting_well_count": len(training_ids),
                    "fitting_ids_sha256": gate._id_digest(sorted(training_ids)),
                },
    ...
    def fake_incumbent_suffixes(
        ...
        selected = well_ids if requested_ids is None else list(requested_ids)
        if not set(selected).issubset(well_ids):
            raise gate.GateError("synthetic incumbent requested unknown well")
        return {
...
def test_success_seals_once_and_independent_audit_reconstructs(
    ...
    result = score.score_structural_field_v2(
        run.protocol,
        run.benchmark,
        run.shard_dir,
        run.barrier,
    ...
    assert result["gate_protocol_version"] == gate.PROTOCOL_VERSION
    assert result["method"] == gate.METHOD
    assert result["fixed_configuration"] == gate.fixed_configuration()
    assert result["lineage"]["v1_stop_parent_lineage_sha256"] == "9" * 64
    ...
    audited = score.audit_score_artifacts_v2(
        run.output,
        run.protocol,
        run.benchmark,
        run.shard_dir,
    ...
    with pytest.raises(score.ScoreError, match="overwrite"):
        score.score_structural_field_v2(
            run.protocol,
            run.benchmark,
            run.shard_dir,
            run.barrier,
    ...
    with pytest.raises(score.ScoreError, match="reconstruct"):
        score.audit_score_artifacts_v2(
            run.output,
            run.protocol,
            run.benchmark,
            run.shard_dir,
...
def test_global_barrier_failures_precede_any_truth_loader(
    ...
    elif attack == "fixed_config":
        run.audit.protocol["fixed_configuration"]["theta_field"] = 0.5
    elif attack == "parent_v1":
        run.audit.protocol["parent_v1_lineage"]["lineage_sha256"] = "not-a-digest"
    else:
...
def test_output_inside_shard_directory_is_rejected_before_truth(
    ...
    frozen_tree_output = (
        run.audit.results_dir
        / gate.EXACT_SHARD_DIR_NAME
        / "fresh_but_forbidden_score.json"
    )
    with pytest.raises(score.ScoreError, match="frozen incumbent/v1 shard trees"):
        score.score_structural_field_v2(
            run.protocol,
            run.benchmark,
            run.shard_dir,
    ...
    assert calls == []
    assert not frozen_tree_output.exists()
